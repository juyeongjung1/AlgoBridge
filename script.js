...
const problems = [
  makeProblem({
    ...
    dummies: [B("combineKanaOnly", "calculation", "氏（かな）と名（かな）だけを結合する"), B("outputLastName", "output", "氏だけを出力する"), B("setFullNameFirst", "assignment", "変数 fullName に、氏の入力値だけを設定する"), ...commonDummies],
  ...
  makeProblem({
    ...
    correct: [B("inputNumber", "input", "整数の入力値を受け取り、変数 number に設定する", "判定する整数を受け取る"), B("checkRemainder", "decision", "変数 number を 2 で割った余りが 0 か判定する"), B("outputEvenOdd", "output", "判定結果として「偶数」または「奇数」を出力する")], dummies: [B("checkDivideTwo", "decision", "入力値を 2 で割った結果が 0 か判定する"), B("outputNumber", "output", "入力値をそのまま出力する"), ...commonDummies],
  ...
  makeProblem({
    ...
    correct: [B("inputA", "input", "1つ目の整数を受け取り、変数 a に設定する", "1つ目の入力値を受け取る"), B("inputB", "input", "2つ目の整数を受け取り、変数 b に設定する", "2つ目の入力値を受け取る"), B("compareValues", "decision", "変数 a が変数 b 以上か判定する"), B("outputLarger", "output", "大きい方の値を出力する")], dummies: [B("compareEqual", "decision", "変数 a と変数 b が等しいか判定する"), B("outputSmaller", "output", "小さい方の値を出力する"), ...commonDummies],
  ...
  makeProblem({
    ...
    correct: [B("inputHour", "input", "時刻を受け取り、変数 hour に設定する", "判定する時刻を受け取る"), B("declareGreeting", "declaration", "挨拶を格納するための変数 greeting を宣言する"), B("selectGreeting", "decision", "変数 hour の範囲に応じて、変数 greeting に挨拶を設定する"), B("outputGreeting", "output", "変数 greeting の値を出力する")], dummies: [B("checkMorningOnly", "decision", "時刻が 12 より小さいかだけ判定する"), B("outputHour", "output", "時刻をそのまま出力する"), ...commonDummies],
  ...
  makeProblem({
    ...
    dummies: [B("initializeSumOne", "assignment", "変数 sum に、初期値として 1 を設定する"), B("loopFromZero", "loop", "ループカウンタ変数 i を 0 から n まで、1 ずつ増やしながら繰り返す", "開始値を考える", true), B("outputCurrent", "output", "ループカウンタ変数 i を、その都度出力する"), B("addToN", "calculation", "変数 n に、ループカウンタ変数 i の値を加える"), ...commonDummies], expected: { root: ["inputN", "declareSum", "initializeSum", "loop", "outputSum"], nested: { loop: ["addCurrent"] } },
