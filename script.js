...
const problems = [
  ...
  makeProblem({
    ...
    correct: [B("inputNumber", "input", "整数の入力値を受け取り、変数 number を宣言すると同時に設定する", "判定する整数を受け取りながら変数を宣言する"), B("checkRemainder", "decision", "変数 number を 2 で割った余りが 0 か判定する", "if の条件を確認する", true, [["true", "条件が真（はい）のとき"], ["false", "条件が偽（いいえ）のとき"]]), B("outputEven", "output", "条件が真のとき、「偶数」を出力する", "if の中で実行する処理"), B("elseOdd", "else", "それ以外の場合の処理を実行する（else）", "if の条件が偽のときの処理をまとめる", true, [["false", "else の中で実行する処理"]]), B("outputOdd", "output", "else の中で、「奇数」を出力する", "条件が偽のときに表示する")], dummies: [B("checkDivideTwo", "decision", "入力値を 2 で割った結果が 0 か判定する"), B("outputEvenOdd", "output", "判定結果として「偶数」または「奇数」をまとめて出力する"), B("outputNumber", "output", "入力値をそのまま出力する"), B("setResultByDivision", "assignment", "変数 result に、入力値を 2 で割った結果を設定する"), B("checkRemainderOne", "decision", "変数 number を 2 で割った余りが 1 か判定する"), B("checkRemainderBelowTwo", "decision", "変数 number を 2 で割った余りが 2 未満か判定する"), B("checkNumberOverTwo", "decision", "変数 number が 2 より大きいか判定する"), ...commonDummies], expected: { root: ["inputNumber", "checkRemainder"], branches: { checkRemainder: { true: ["outputEven"], false: ["elseOdd"] }, elseOdd: { false: ["outputOdd"] } } },
    ...
    explanation: [["判定する整数を受け取る", "最初に入力値を number に設定します。判定処理では、この変数の値を使います。"], ["if の条件を確認する", "偶数は 2 で割り切れる数なので、number を 2 で割った余りが 0 かを調べます。"], ["if の中では「偶数」を出力する", "余りが 0 なら条件は真です。if ブロックの中で「偶数」を表示します。"], ["else の中では「奇数」を出力する", "余りが 0 でなければ条件は偽です。else ブロックの中で「奇数」を表示します。"]],
    code: [["inputNumber", "int number = 8;  // 整数の入力値", "整数を受け取る", "入力された整数を number に設定します。"], ["checkRemainder", "if (number % 2 == 0) {", "if文と %", "% は割り算の余りを求めます。if は条件が成り立つときに処理を選ぶ文です。"], ["outputEven", "    System.out.println(\"偶数\");", "if の中の処理", "条件が真の場合に、if の波かっこの中で偶数を表示します。"], ["elseOdd", "} else {", "else とは", "else は if の条件が成り立たない場合の処理を始める部分です。"], ["outputOdd", "    System.out.println(\"奇数\");", "else の中の処理", "条件が偽の場合に、else の波かっこの中で奇数を表示します。"], ["checkRemainder", "}", "波かっこ", "if / else 全体の処理の終わりを表します。"]]
  }),
  makeProblem({
...
function snapshot() { return [...assemblyList.children].filter((x) => x.classList.contains("placed-block")).map((x) => ({ id: x.dataset.blockId, children: x.querySelector(".loop-body") ? directIds(x.querySelector(".loop-body")) : [], branches: Object.fromEntries([...x.querySelectorAll(":scope > .branch-bodies > .branch-body")].map((branch) => [branch.dataset.branch, directIds(branch)])) })); }
function saveHistory() { history.push(snapshot()); }
function restore(snap) { assemblyList.replaceChildren(); snap.forEach((entry) => { const block = createBlock(entry.id); assemblyList.append(block); const zone = block.querySelector(".loop-body"); (entry.children || []).forEach((id) => { zone.querySelector(":scope > [data-empty]")?.remove(); zone.append(createBlock(id)); }); Object.entries(entry.branches || {}).forEach(([name, ids]) => { const branch = block.querySelector(`.branch-body[data-branch="${name}"]`); ids.forEach((id) => { branch.querySelector(":scope > [data-empty]")?.remove(); branch.append(createBlock(id)); }); }); }); refreshAssembly(); }
...
function flowClass(block) {
  ...
  if (block.type === "else") return "else-node";
...
function appendFlowBlock(container, id) {
  ...
  if (branches) {
    ...
    branchEntries.forEach(([name, branchIds]) => {
      ...
      branch.innerHTML = `<strong>${name === "true" ? "真（はい）" : "偽（いいえ）"}</strong>`;
