...
const problems = [
  ...
  makeProblem({
    id: "max", category: "選択", title: "if文で2つの値を比べる", inputNote: "2つの整数を入力してください。", description: "2つの整数を受け取り、変数 <code>a</code> が変数 <code>b</code> より大きい場合は <code>a</code> を、それ以外の場合は <code>b</code> を表示してください。", inputs: [I("a", "入力値 a", 12), I("b", "入力値 b", 7)],
    unorderedPrefix: ["inputA", "inputB"],
    correct: [B("inputA", "input", "1つ目の整数を受け取り、変数 a を宣言すると同時に設定する", "1つ目の入力値を準備する"), B("inputB", "input", "2つ目の整数を受け取り、変数 b を宣言すると同時に設定する", "2つ目の入力値を準備する"), B("compareValues", "decision", "変数 a が変数 b より大きいか判定する", "if の条件を確認する", true, [["true", "条件が真（はい）のとき"], ["false", "条件が偽（いいえ）のとき"]]), B("outputA", "output", "変数 a の値を出力する", "真の経路で実行する処理"), B("outputB", "output", "変数 b の値を出力する", "偽の経路で実行する処理")], dummies: [B("compareEqual", "decision", "変数 a と変数 b が等しいか判定する"), B("compareLess", "decision", "変数 a が変数 b より小さいか判定する", "if の条件を確認する", true, [["true", "条件が真（はい）のとき"], ["false", "条件が偽（いいえ）のとき"]]), B("compareSumToZero", "decision", "変数 a と変数 b の合計が 0 か判定する"), B("checkAIsPositive", "decision", "変数 a が 0 より大きいか判定する"), B("outputSmaller", "output", "小さい方の値を出力する"), B("setResultByAddition", "assignment", "変数 result に、変数 a と変数 b を足した値を設定する"), B("outputInputOrder", "output", "入力した順番のまま、変数 a と変数 b を出力する"), ...commonDummies], expected: { root: ["inputA", "inputB", "compareValues"], branches: { compareValues: { true: ["outputA"], false: ["outputB"] } } },
    execute(v) { const a = Number(v.a), b = Number(v.b), output = a > b ? a : b; return { output, traceColumns: ["a", "b", "判定結果", "出力値"], trace: [[a, b, a > b ? "a > b（真）" : "a > b（偽）", output]] }; },
    explanation: [["2つの整数を受け取る", "比較する値を a と b に設定します。この2つの入力順は入れ替わっても構いません。"], ["a が b より大きいか判定する", "if 文では、条件が成り立つと真の経路、成り立たないと偽の経路へ進みます。ここでは a > b を調べます。"], ["真の経路では a を出力する", "a が b より大きいときは、問題文の指定どおり a の値を表示します。"], ["偽の経路では b を出力する", "a が b より大きくないときは、b の値を表示します。a と b が同じ場合も、この経路で b を表示します。"]],
    code: [["inputA", "int a = 12;  // 1つ目の入力値", "変数 a", "1つ目の整数を受け取り、int 型の変数 a に設定します。"], ["inputB", "int b = 7;  // 2つ目の入力値", "変数 b", "2つ目の整数を受け取り、int 型の変数 b に設定します。"], ["compareValues", "if (a > b) {", "if文と比較演算子 >", "a が b より大きいときだけ、波かっこの中を実行します。"], ["outputA", "    System.out.println(a);", "真の経路の処理", "条件が真の場合に、a の値を表示します。"], ["outputB", "} else {", "偽の経路への切替", "条件が偽の場合は、else により偽の経路の処理を実行します。"], ["outputB", "    System.out.println(b);", "偽の経路の処理", "条件が偽の場合に、b の値を表示します。"], ["compareValues", "}", "波かっこ", "if / else 全体の処理の終わりを表します。"]]
  }),
  makeProblem({
  ...
  makeProblem({
    ...
    correct: [B("inputHour", "input", "時刻を受け取り、変数 hour を宣言すると同時に設定する", "時刻を受け取りながら変数を宣言する"), B("declareGreeting", "declaration", "挨拶を格納するための変数 greeting を宣言する"), B("checkMorning", "decision", "変数 hour が 0 以上 11 以下か判定する", "最初の条件を確認する", true, [["true", "条件が真（はい）のとき"], ["false", "条件が偽（いいえ）のとき"]]), B("setMorning", "assignment", "変数 greeting に文字列「おはようございます」を設定する"), B("checkAfternoon", "decision", "変数 hour が 12 以上 17 以下か判定する", "次の時間帯の条件を確認する", true, [["true", "条件が真（はい）のとき"], ["false", "条件が偽（いいえ）のとき"]]), B("setAfternoon", "assignment", "変数 greeting に文字列「こんにちは」を設定する"), B("checkEvening", "decision", "変数 hour が 18 以上 23 以下か判定する", "最後の時間帯の条件を確認する", true, [["true", "条件が真（はい）のとき"], ["false", "条件が偽（いいえ）のとき"]]), B("setEvening", "assignment", "変数 greeting に文字列「こんばんは」を設定する"), B("setHourGuide", "assignment", "変数 greeting に文字列「0〜23時で指定してください」を設定する"), B("outputGreeting", "output", "変数 greeting の値を出力する")], dummies: [B("checkMorningOnly", "decision", "時刻が 12 より小さいかだけ判定する"), B("wrongElseIf", "decision", "変数 hour が 12 より小さいか判定する", "次の時間帯の条件を確認する"), B("outputHour", "output", "時刻をそのまま出力する"), B("setResultAsHour", "assignment", "変数 result に、入力された時刻を設定する"), B("setMorningForAfternoon", "decision", "変数 hour が 12〜17 のとき、「おはようございます」を変数 greeting に設定する"), B("setHelloAfterNoon", "decision", "変数 hour が 12 以上なら、「こんにちは」を変数 greeting に設定する"), B("setGreetingForValidHour", "decision", "変数 hour が 0〜23 のとき、「こんにちは」を変数 greeting に設定する"), ...commonDummies], expected: { root: ["inputHour", "declareGreeting", "checkMorning", "outputGreeting"], branches: { checkMorning: { true: ["setMorning"], false: ["checkAfternoon"] }, checkAfternoon: { true: ["setAfternoon"], false: ["checkEvening"] }, checkEvening: { true: ["setEvening"], false: ["setHourGuide"] } } },
  ...
  makeProblem({
    ...
    correct: [B("inputYear", "input", "西暦を受け取り、変数 year を宣言すると同時に設定する", "西暦を受け取りながら変数を宣言する"), B("declareLeap", "declaration", "判定結果を格納するための変数 leapYear を宣言する"), B("checkFourHundred", "decision", "変数 year が 400 で割り切れるか判定する", "最初の条件を確認する", true, [["true", "条件が真（はい）のとき"], ["false", "条件が偽（いいえ）のとき"]]), B("setLeapByFourHundred", "assignment", "変数 leapYear に文字列「うるう年です」を設定する"), B("checkOneHundred", "decision", "変数 year が 100 で割り切れるか判定する", "100年の例外を確認する", true, [["true", "条件が真（はい）のとき"], ["false", "条件が偽（いいえ）のとき"]]), B("setCommonByOneHundred", "assignment", "変数 leapYear に文字列「うるう年ではありません」を設定する"), B("checkFour", "decision", "変数 year が 4 で割り切れるか判定する", "通常の4年ごとの条件を確認する", true, [["true", "条件が真（はい）のとき"], ["false", "条件が偽（いいえ）のとき"]]), B("setLeapByFour", "assignment", "変数 leapYear に文字列「うるう年です」を設定する"), B("setCommonByDefault", "assignment", "変数 leapYear に文字列「うるう年ではありません」を設定する"), B("outputLeap", "output", "変数 leapYear の値を出力する")], dummies: [B("checkFourOnly", "decision", "4 で割り切れるかだけを判定する"), B("wrongLeapElseIf", "decision", "変数 year が 4 で割り切れるか判定する", "次の条件を確認する"), B("checkHundredFirst", "decision", "100 で割り切れる年を必ずうるう年にする"), B("setResultByFour", "assignment", "変数 result に、変数 year を 4 で割った結果を設定する"), B("checkFourOrHundred", "decision", "4 または 100 で割り切れる年を、うるう年に設定する"), B("setFourHundredAsCommonYear", "decision", "400 で割り切れる年を、うるう年ではないと設定する"), B("checkRemainderOneForLeap", "decision", "変数 year を 4 で割った余りが 1 か判定する"), ...commonDummies], expected: { root: ["inputYear", "declareLeap", "checkFourHundred", "outputLeap"], branches: { checkFourHundred: { true: ["setLeapByFourHundred"], false: ["checkOneHundred"] }, checkOneHundred: { true: ["setCommonByOneHundred"], false: ["checkFour"] }, checkFour: { true: ["setLeapByFour"], false: ["setCommonByDefault"] } } },
...
const problemHints = {
  ...
  max: [
    { simple: "問題文は、a が b より大きいかどうかで、表示する値を選ぶよう指定しています。", concrete: "「変数 a が変数 b より大きいか判定する」ブロックを置きます。a が b より小さいか、等しいか、0より大きいかを調べても、問題文の条件とは一致しません。" },
    { simple: "条件が真の場合と偽の場合で、出力する変数は入れ替わります。", concrete: "真の経路には「変数 a の値を出力する」、偽の経路には「変数 b の値を出力する」を直接配置します。" },
    { simple: "比較する前に、2つの入力値を準備する必要があります。", concrete: "入力値 a と入力値 b を受け取り、変数を宣言すると同時に設定します。合計を計算したり、2つの値を続けて出力したりする処理は必要ありません。" }
  ],
  weekday: [
    { simple: "1つの数値に応じて多くの処理を選ぶときは、各場合を分けて考えます。", concrete: "「day の値に応じて分岐する（switch）」を選びます。「翌日の曜日を設定する」は day が 0 のとき月曜日になってしまい、「7で割った余り」は 7 を日曜日として扱ってしまうため、どちらも問題文の指定と異なります。" },
    { simple: "どの経路でも、最後に表示する変数へ文字列を設定する必要があります。", concrete: "0〜6 の各領域には対応する曜日、それ以外の領域には「Error」を weekday に設定します。「day が 0 以上かだけを判定する」では、1と6を別々の曜日へ分けられず、負の値以外を正しく扱えません。" },
    { simple: "曜日を選ぶ処理と、表示する処理は分けられています。", concrete: "switch の後に、変数 weekday の値を出力します。「入力値をそのまま出力する」では 1 のままで「月曜日」にならず、「result に曜日の番号を設定する」も表示すべき文字列を保存できません。" }
  ],
  greeting: [
...
function updateZoneDropSpace(zone) { const nested = zone !== assemblyList; const placedBlocks = [...zone.querySelectorAll(":scope > .placed-block")]; const hasBlocks = placedBlocks.length > 0; const lastBlock = placedBlocks.at(-1); const endsWithNestedBranch = nested && lastBlock?.classList.contains("has-branch-body"); const empty = zone.querySelector(":scope > [data-empty]"); let space = zone.querySelector(":scope > .drop-space"); if (!hasBlocks || endsWithNestedBranch) { space?.remove(); if (!hasBlocks && !empty) createEmpty(zone, nested); return; } empty?.remove(); if (!space) space = createDropSpace(zone, nested); zone.append(space); }
