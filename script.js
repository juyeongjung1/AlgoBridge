...
const problems = [
  ...
  makeProblem({
    ...
    explanation: [["判定する整数を受け取る", "最初に入力値を number に設定します。判定処理では、この変数の値を使います。"], ["余りで判定する", "偶数は 2 で割り切れる数なので、2 で割った余りが 0 かを調べます。"], ["真のときは「偶数」を出力する", "余りが 0 なら条件は真です。この経路では「偶数」を表示します。"], ["偽のときは「奇数」を出力する", "余りが 0 でなければ条件は偽です。この経路では「奇数」を表示します。"]],
    code: [["inputNumber", "int number = 8;  // 整数の入力値", "整数を受け取る", "入力された整数を number に設定します。"], ["checkRemainder", "if (number % 2 == 0) {", "if文と %", "% は割り算の余りを求めます。if は条件が成り立つときに処理を選ぶ文です。"], ["outputEven", "    System.out.println(\"偶数\");", "条件が真のとき", "条件が成り立つ場合に偶数を表示します。"], ["outputOdd", "} else {", "else とは", "else は if の条件が成り立たない場合の処理を始める部分です。"], ["outputOdd", "    System.out.println(\"奇数\");", "条件が偽のとき", "条件が成り立たない場合に奇数を表示します。"], ["checkRemainder", "}", "波かっこ", "if文の処理の終わりを表します。"]]
  }),
  makeProblem({
    ...
    correct: [B("inputA", "input", "1つ目の整数を受け取り、変数 a を宣言すると同時に設定する", "1つ目の入力値を受け取りながら変数を宣言する"), B("inputB", "input", "2つ目の整数を受け取り、変数 b を宣言すると同時に設定する", "2つ目の入力値を受け取りながら変数を宣言する"), B("compareValues", "decision", "変数 a が変数 b 以上か判定する"), B("outputLarger", "output", "大きい方の値を出力する")], dummies: [B("compareEqual", "decision", "変数 a と変数 b が等しいか判定する"), B("outputSmaller", "output", "小さい方の値を出力する"), B("setResultByAddition", "assignment", "変数 result に、変数 a と変数 b を足した値を設定する"), B("compareLessOrEqual", "decision", "変数 a が変数 b 以下か判定する"), B("compareSumToZero", "decision", "変数 a と変数 b の合計が 0 か判定する"), B("checkAIsPositive", "decision", "変数 a が 0 以上か判定する"), ...commonDummies],
  ...
  makeProblem({
    ...
    correct: [B("inputDay", "input", "曜日の番号を受け取り、変数 day を宣言すると同時に設定する", "曜日の番号を受け取りながら変数を宣言する"), B("declareWeekday", "declaration", "曜日を格納するための変数 weekday を宣言する"), B("selectWeekday", "decision", "変数 day の値に応じて、曜日または Error を変数 weekday に設定する"), B("outputWeekday", "output", "変数 weekday の値を出力する")], dummies: [B("loopWeekday", "loop", "0 から 6 まで繰り返して曜日を表示する"), B("outputDay", "output", "入力値をそのまま出力する"), B("setResultAsDayNumber", "assignment", "変数 result に、曜日の番号をそのまま設定する"), B("selectNextWeekday", "decision", "変数 day の値に 1 を足して、対応する曜日を変数 weekday に設定する"), B("selectWeekdayByRemainder", "decision", "変数 day を 7 で割った余りに応じて、変数 weekday に曜日を設定する"), B("checkDayIsNonNegative", "decision", "変数 day が 0 以上かだけを判定して、変数 weekday に曜日を設定する"), ...commonDummies],
  ...
  makeProblem({
    ...
    correct: [B("inputHour", "input", "時刻を受け取り、変数 hour を宣言すると同時に設定する", "時刻を受け取りながら変数を宣言する"), B("declareGreeting", "declaration", "挨拶を格納するための変数 greeting を宣言する"), B("selectGreeting", "decision", "変数 hour の範囲に応じて、変数 greeting に挨拶を設定する"), B("outputGreeting", "output", "変数 greeting の値を出力する")], dummies: [B("checkMorningOnly", "decision", "時刻が 12 より小さいかだけ判定する"), B("outputHour", "output", "時刻をそのまま出力する"), B("setResultAsHour", "assignment", "変数 result に、入力された時刻を設定する"), B("setMorningForAfternoon", "decision", "変数 hour が 12〜17 のとき、「おはようございます」を変数 greeting に設定する"), B("setHelloAfterNoon", "decision", "変数 hour が 12 以上なら、「こんにちは」を変数 greeting に設定する"), B("setGreetingForValidHour", "decision", "変数 hour が 0〜23 のとき、「こんにちは」を変数 greeting に設定する"), ...commonDummies],
...
function expectedIds() { return [...currentProblem.expected.root, ...Object.values(currentProblem.expected.nested || {}).flat(), ...Object.values(currentProblem.expected.branches || {}).flatMap((branch) => Object.values(branch).flat())]; }
...
function zoneMessage(zone, next = false) { if (zone.classList.contains("decision-branch")) return next ? "次の分岐処理をここに配置" : "ここに出力処理を配置"; return next ? "次の繰り返し処理をここに配置" : "ここに繰り返しの中の処理を配置"; }
...
function createBlock(id) { const block = definition(id), item = document.createElement("article"); item.className = `placed-block block-${block.type}`; item.draggable = true; item.dataset.blockId = id; item.innerHTML = `<div class="placed-copy"><strong>${formatLabel(block.label)}</strong><span>${escapeHtml(block.hint)}</span><button class="remove-block" type="button" aria-label="削除">×</button></div>`; item.querySelector(".remove-block").addEventListener("click", () => { saveHistory(); item.remove(); refreshAssembly(); }); item.addEventListener("dragstart", () => { dragged = { id, source: "placed", item }; }); if (block.type === "loop" && block.acceptsChildren) { const body = document.createElement("div"); body.className = "loop-body dropzone"; body.dataset.zone = id; body.innerHTML = '<span class="loop-body-label">繰り返しの中で実行する処理</span>'; createEmpty(body, true); addDropEvents(body); item.append(body); const end = document.createElement("div"); end.className = "loop-end-symbol"; end.innerHTML = "<span>繰り返し終了</span>"; item.append(end); } if (block.type === "decision" && block.acceptsChildren) { item.classList.add("has-decision-branches"); const branches = document.createElement("div"); branches.className = "decision-branches"; [["true", "真（はい）のとき"], ["false", "偽（いいえ）のとき"]].forEach(([name, label]) => { const branch = document.createElement("div"); branch.className = `decision-branch decision-branch-${name} dropzone`; branch.dataset.zone = `${id}:${name}`; branch.dataset.branch = name; branch.innerHTML = `<span class="decision-branch-label">${label}</span>`; createEmpty(branch, true); addDropEvents(branch); branches.append(branch); }); item.append(branches); } return item; }
...
function refreshAssembly() { [assemblyList, ...assemblyList.querySelectorAll(".loop-body, .decision-branch")].forEach(updateZoneDropSpace); updateAssemblySymbols(); updatePalette(); clearFeedback(); clearResults(); }
...
function snapshot() { return [...assemblyList.children].filter((x) => x.classList.contains("placed-block")).map((x) => ({ id: x.dataset.blockId, children: x.querySelector(".loop-body") ? directIds(x.querySelector(".loop-body")) : [], branches: Object.fromEntries([...x.querySelectorAll(":scope > .decision-branches > .decision-branch")].map((branch) => [branch.dataset.branch, directIds(branch)])) })); }
function saveHistory() { history.push(snapshot()); }
function restore(snap) { assemblyList.replaceChildren(); snap.forEach((entry) => { const block = createBlock(entry.id); assemblyList.append(block); const zone = block.querySelector(".loop-body"); (entry.children || []).forEach((id) => { zone.querySelector(":scope > [data-empty]")?.remove(); zone.append(createBlock(id)); }); Object.entries(entry.branches || {}).forEach(([name, ids]) => { const branch = block.querySelector(`.decision-branch[data-branch="${name}"]`); ids.forEach((id) => { branch.querySelector(":scope > [data-empty]")?.remove(); branch.append(createBlock(id)); }); }); }); refreshAssembly(); }
...
function appendFlowBlock(container, id) {
  ...
  if (branches) {
    ...
    branchGroup.className = "flow-decision-branches";
    [["true", "真（はい）"], ["false", "偽（いいえ）"]].forEach(([name, label]) => {
      const branch = document.createElement("div");
      branch.className = `flow-decision-branch flow-decision-branch-${name}`;
      branch.innerHTML = `<strong>${label}</strong>`;
      branches[name].forEach((branchId) => {
        appendFlowArrow(branch);
        const branchBlock = definition(branchId);
        branch.append(createFlowNode(branchId, branchBlock.label, flowClass(branchBlock)));
      });
